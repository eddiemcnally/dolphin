...
pub fn get_move_list() -> Vec<Mov> {
    return Vec::with_capacity(MOVE_LIST_SIZE);
}

const MOVE_LIST_SIZE: usize = 128;
...
fn generate_pawn_moves(pos: &Position, move_list: &mut Vec<Mov>) {
    ...
    let opposite_side = side.flip_side();
    ...
    let pawn_bb = match side {
        Colour::White => pos.board().get_piece_bitboard(Piece::WhitePawn),
        Colour::Black => pos.board().get_piece_bitboard(Piece::BlackPawn),
    };

    // =================================================
    // Pawn quiet + capture, exclude possible promotions
    // =================================================
    let mut excl_pawn_bb = match side {
    ...
    while excl_pawn_bb != 0 {
        ...
        let (quiet_to_sq, capt_mask) = if side == Colour::White {
            (
                from_sq.square_plus_1_rank(),
                pos.occupancy_masks()
                    .get_occ_mask_white_pawn_attack_squares(from_sq),
            )
        } else {
            (
                from_sq.square_minus_1_rank(),
                pos.occupancy_masks()
        ...
        match quiet_to_sq {
            ...
            None => panic!("Invalid square when trying for quiet move"),
        }
    ...
    // double pawn move
    // ================
    let mut starting_pawn_bb = match side {
    ...
    while starting_pawn_bb != 0 {
        ...
        let (single_mv_to_sq, double_mv_to_sq) = match side {
            Colour::White => (from_sq.square_plus_1_rank(), from_sq.square_plus_2_ranks()),
            Colour::Black => (
                from_sq.square_minus_1_rank(),
                from_sq.square_minus_2_ranks(),
            ),
        };
        ...
        match double_mv_to_sq {
            ...
            None => panic!("Problem trying to do pawn double move"),
        }
    }
    ...
    while promo_bb != 0 {
        ...
        match quiet_to_sq {
            ...
            None => panic!("Problem with promotion trying to do pawn promotion"),
        }
...
fn generate_sliding_diagonal_antidiagonal_moves(pos: &Position, move_list: &mut Vec<Mov>) {
    let board = pos.board();
...
// see Hyperbola Quintessence
fn generate_sliding_rank_file_moves(pos: &Position, move_list: &mut Vec<Mov>) {
    let mut pce_bb = if pos.side_to_move() == Colour::White {
...
fn generate_non_sliding_piece_moves(pos: &Position, move_list: &mut Vec<Mov>) {
